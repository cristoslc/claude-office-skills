...
def validate_excel_file(filename):
    """Validate that filename is safe to process and is a valid Excel file."""
    p = Path(filename).resolve()
    cwd = Path.cwd().resolve()
    try:
        p.relative_to(cwd)
    except ValueError:
        raise ValueError(f"File must be within current directory: {filename}")
    if not p.is_file():
        raise ValueError(f"Not a valid file: {filename}")
    if p.suffix.lower() not in (".xlsx", ".xls", ".ods"):
...
def recalc(filename, timeout=30):
    ...
    """
    try:
        abs_path = validate_excel_file(filename)
